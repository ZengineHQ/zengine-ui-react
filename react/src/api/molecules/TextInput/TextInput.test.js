...
test('Changes input id when specified', () => {
  const { container } = render(<MockForm><TextInput name="test" id="whoathere"/></MockForm>);
...
test('Displays custom help when specified', () => {
  const { container } = render(<MockForm><TextInput label="Foo" name="foo" help="foo bar" /></MockForm>);
