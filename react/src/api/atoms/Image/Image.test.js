...
test('Renders an img HTML tag', () => {
  const { container } = render(<Image alt="test image" src="Yidhra-face.jpg"/>);
...
test('Sets height when specified', () => {
  const { container } = render(<Image alt="test image" src="Yidhra-face.jpg" height="40" />);
  expect(container.firstChild).toHaveAttribute('height', '40');
});

test('Sets width when specified', () => {
  const { container } = render(<Image alt="test image" src="Yidhra-face.jpg" width="20" />);
  expect(container.firstChild).toHaveAttribute('width', '20');
});

test('Adds custom classes when specified', () => {
  const { container } = render(<Image alt="test image" src="Yidhra-face.jpg" classes="foo bar baz" />);
